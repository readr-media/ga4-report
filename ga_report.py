...
def recent_popular_report(property_id, dest_file='popular.json', days: int=1):
    ...
    posts_gql = '''
        query{{
            posts(where: {{publishedDate: {{gt: {START_DATE} }}, state: {{equals: "published"}} }}){{
                id
                slug
                sections{{id, name, slug, state}}
                sectionsInInputOrder{{id, name, slug, state}}
    ...
    print(f"[recent_popular_report] days={days} candidates={len(filter_slugs)} "
          f"with_views={len(statistic_data)} report={len(report)}", flush=True)
